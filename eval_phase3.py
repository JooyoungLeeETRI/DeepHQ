...
def compute_bpp(out_net):
    size = out_net['x_hat'].size()
    num_pixels = size[0] * size[2] * size[3]
    mask = out_net.get("mask", None)

    total_logprob = 0.0
    for likelihoods in out_net['likelihoods'].values():
        log_lik = torch.log(likelihoods)
        # Multiply by mask only when shapes match exactly
        if mask is not None and mask.shape == likelihoods.shape:
            # print("same shape")
            log_lik = log_lik * mask
        total_logprob += log_lik.sum()

    return (total_logprob / (-math.log(2) * num_pixels)).item()
...
def main(argv):
    ...
    for q_idx in range(TOTAL_Q):
        print(f'Quality level:{q_idx} ############')
        print(f'average_Mask ratio: {Mask_ratio[q_idx]:.4f}')
