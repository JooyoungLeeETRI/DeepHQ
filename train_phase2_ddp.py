...
def main_worker(rank, world_size, args):
    ...
    model = TCM_Phase2(config=[2,2,2,2,2,2], head_dim=[8, 16, 32, 32, 16, 8], drop_path_rate=0.0, N=args.N, M=320)
    ...
    for epoch in range(last_epoch, args.epochs):
        if epoch == args.epochs - 4:
            for param_group in optimizer.param_groups:
                param_group['lr'] = 1e-5
            print(f"[Epoch {epoch}] LR manually set to 1e-5")
        elif epoch == args.epochs - 2:
            for param_group in optimizer.param_groups:
                param_group['lr'] = 1e-6
            print(f"[Epoch {epoch}] LR manually set to 1e-6")
